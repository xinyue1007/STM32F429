...
uint8_t SDRAM_Send_Cmd(uint8_t bankx, uint8_t cmd, uint8_t refresh, uint16_t regval)
{
  ...
  Command.CommandMode = cmd;                                       //?¡®???¡è
  Command.CommandTarget = target_bank;                             //??????SDRAM?-???¡§??o???
  Command.AutoRefreshNumber = refresh;                             //¨¨?a??¡¤?¨C¡ã???????
...
void MX_FMC_Init(void)
{
  ...
  */
  ...
  SDRAM_Initialization_Sequence(&hsdram1);     //??¡®???????SDRAM????¡ì???¨C?o?????
  HAL_SDRAM_ProgramRefreshRate(&hsdram1, 683); //¨¨???????¡¤?¨C¡ã¨¦¡é¡®???
